...
export class MoviesController {
  constructor(private moviesService: MoviesService) {}
     ...
     * @param apiKey - The API key for authentication.
     * @param date - The date to retrieve the box office data (in YYYYMMDD format).
     * @example http://localhost:3000/movies/weekly?apiKey=${apiKey}&date=20230920
     */
    @Get('weekly')
    async getWeeklyBoxOffice(@Query('apiKey') apiKey: string, @Query('date') date: string) {
      return this.moviesService.getWeeklyBoxOffice(apiKey, date);
    }
}